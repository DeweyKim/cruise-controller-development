...
int8_t main(int8_t argc, char* argv[]) {
    ...
    static int8_t cruise_mode_status = 0;
    ...
    while (STATUS_KEY != InputKey_Exit) {
        if (_kbhit()) {
            ...
            if ((STATUS_KEY == InputKey_SetAccel_Short) && (cruise_mode_status == 1)) {
                printf("목표속도:%d\n", Accelerate(1, 0));
            }

            if ((STATUS_KEY == InputKey_SetAccel_Long) && (cruise_mode_status == 1)) {
                printf("목표속도:%d\n", Accelerate(0, 1));
            }

            if ((STATUS_KEY == InputKey_ResDecel_Short) && (cruise_mode_status == 1)) {
                printf("목표속도:%d\n", Decelerate(1, 0));
            }
            if ((STATUS_KEY == InputKey_ResDecel_Long) && (cruise_mode_status == 1)) {
                printf("목표속도:%d\n", Decelerate(0, 1));
            }

            if (((STATUS_KEY == InputKey_SetAccel_Long) || (STATUS_KEY == InputKey_SetAccel_Short) || (STATUS_KEY == InputKey_ResDecel_Long) || (STATUS_KEY == InputKey_ResDecel_Short)) && (cruise_mode_status == 0)) {
                printf("Cruise Mode Off 상태입니다.\n");
            }
            if (STATUS_KEY == InputKey_Resume && cruise_mode_status == 0) {
                cruise_btn = 1;
                cancel_btn = 0;
                car_brake = 0;
            ...
            if (STATUS_KEY == InputKey_Exit) {
                break;
            }

            if (STATUS_KEY == InputKey_Cancel) {
                car_brake = 0;
                cancel_btn = 1;
                cruise_btn = 0;
                cruise_mode_status = cruise_mode(current_spd, car_brake, cancel_btn, cruise_btn, is_fault);
                printf("종료를 선택하셨습니다. Cruise Mode를 종료합니다\n");
                //_getch();

            }
            if (STATUS_KEY == InputKey_Brake) {
                car_brake = 1;
                cancel_btn = 0;
                cruise_btn = 0;
                cruise_mode_status = cruise_mode(current_spd, car_brake, cancel_btn, cruise_btn, is_fault);
                printf("Brake를 밟습니다. Cruise Mode를 종료합니다.\n");
                //_getch();
